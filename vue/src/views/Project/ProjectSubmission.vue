  ...
  methods:{
    async fetchData() {
      ...
      await request.get('/date/list').then( res=> {
        let today = new Date()
        let begin = new Date(res.data.submissionBegin)
        let end = new Date(res.data.submissionEnd)
        //如果当前日期不在课题申报的开始和截止日期，将condition设置为true，只有false才能进行申报
        if(today < end && today > begin){
          this.condition = true
        }
      ...
      await request.get('/process/listProcess',{params:that.params}).then(res =>{
        if (res.code === '200'){
          ...
          that.tableData.forEach((item) =>{
            //当tableData不为审核驳回时显示查看,否则显示提交
            item.condition = item.processCondition !== '课题申报审核驳回';
            item.userType=2
        ...
        //默认添加、如果查询到任意一条信息或不在日期内则不添加
        if(res.code !== '200' && this.condition){
